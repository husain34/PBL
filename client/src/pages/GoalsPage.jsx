...
function projectedCompletion(goal) {
  ...
  const daysSinceCreation = Math.max(1,
    Math.ceil((new Date() - new Date(goal.createdAt)) / (1000 * 60 * 60 * 24))
  );
  const dailyRate = goal.currentProgress / daysSinceCreation;
  if (dailyRate <= 0) return "No allocation yet";
...
const EMPTY_FORM = {
  name: "", targetAmount: "", deadline: "", priority: "Medium", color: "#3b82f6",
};

export default function GoalsPage() {
  const [goals, setGoals] = useState([]);
  const [savingsPot, setSavingsPot] = useState(0);
  ...
  const fetchGoals = useCallback(async () => {
    try {
      ...
      setSavingsPot(res.data.savingsPot);
  ...
  return (
    <div className="min-h-screen bg-background text-foreground px-4 py-10 max-w-6xl mx-auto space-y-8">
      ...
      <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-6 py-4 flex items-center justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-emerald-600 mb-1">Savings Pot</p>
          <p className="text-3xl font-bold text-emerald-700">{formatFullINR(savingsPot)}</p>
      ...
      {showForm && (
        <div className="auth-card max-w-lg space-y-4">
          ...
          <button
            onClick={handleAddGoal} disabled={submitting}
              ...
              return (
                <div key={goal._id} className={`rounded-2xl border p-5 space-y-4 ${p.bg} ${p.border}`}>
                  <div className="flex items-start justify-between">
                    ...
                    <button onClick={() => handleDeleteGoal(goal._id)} className="text-xs text-muted-foreground hover:text-red-500 transition-colors">×</button>
                  </div>
      ...
      {goals.length === 0 && (
        <div className="auth-card flex flex-col items-center justify-center py-16 text-center space-y-3">
          ...
          <p className="text-sm text-muted-foreground max-w-xs">Create your first financial goal and start allocating from your savings pot.</p>
        </div>
      )}
    </div>
  );
