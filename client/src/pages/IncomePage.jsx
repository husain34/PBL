    ...
    <div className="min-h-screen bg-background text-foreground px-4 py-10 max-w-6xl mx-auto space-y-8">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1 text-sm">
          Log and monitor all your income sources in one place.
        </p>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <SummaryCard
            ...
            value={summary.distinctSources}
            color="violet"
            sub="Distinct sources logged"
          />
        </div>
      )}

      {/* Profile income vs logged nudge */}
      {summary?.profileIncome && summary.thisMonthTotal > 0 && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 flex items-start gap-2">
          <span>
            You declared <strong>{summary.profileIncome}</strong> as your income
            range during setup. You've logged{" "}
          ...
          {!hasData ? (
            <div className="auth-card flex flex-col items-center justify-center py-16 text-center space-y-3">
              ...
              <p className="text-sm text-muted-foreground max-w-xs">
                Add your first income entry using the form to start seeing your
              ...
              <div className="auth-card">
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={summary?.monthlyChart || []}>
                    ...
                    <Tooltip
                      formatter={(v) => [formatFullINR(v), "Income"]}
                ...
                {entries.slice(0, 10).map((entry) => (
                  <tr key={entry._id} className="hover:bg-secondary/40 transition-colors">
                    ...
                    <td className="py-2.5 pr-4 font-semibold">
                      {formatFullINR(entry.amount)}
                    </td>
                    <td className="py-2.5 pr-4 text-muted-foreground text-xs">
                      {entry.frequency}
                    </td>
                    <td className="py-2.5 pr-4 text-muted-foreground max-w-[160px] truncate">
                      {entry.note || "—"}
                    </td>
                    <td className="py-2.5">
                      <button
